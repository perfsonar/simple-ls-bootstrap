...
SimpleLSBootStrapClientDaemon.pl - Detrmines lookup service to be used by this host
...
use SimpleLSBootStrap::ClientDaemonHandler;
...
$0 = "SimpleLSBootStrapClientDaemon.pl";
...
if ( not $PIDFILE ) {
    $PIDFILE = "/var/run/SimpleLSBootStrapClientDaemon.pid";
}
...
$logger->info( perfSONAR_PS::Utils::NetLogger::format( "SimpleLSBootStrapClientDaemon.init.start") );
if ( not $conf{"hosts_file"} ) {
    my $log_msg = perfSONAR_PS::Utils::NetLogger::format( "SimpleLSBootStrapClientDaemon.init.end", 
        { status => -1, 
...
#BEGIN handler
my $handler = new SimpleLSBootStrap::ClientDaemonHandler();
$handler->init( \%conf );
$logger->info( perfSONAR_PS::Utils::NetLogger::format( "SimpleLSBootStrapClientDaemon.init.end") );
...
L<perfSONAR_PS::Utils::Host>, L<SimpleLSBootStrap::ClientDaemonHandler>
